...
// ========== HTML এসকেপ ফাংশন (সুরক্ষার জন্য) ==========
...
// ========== নেভিগেশন ==========
const navItems = [
    { id: 'dashboardPage', icon: '📊', label: 'ড্যাশবোর্ড' },
    { id: 'addPage', icon: '➕', label: 'যোগ করুন' },
    { id: 'transactionsPage', icon: '📋', label: 'লেনদেন' },
    { id: 'reportsPage', icon: '📅', label: 'রিপোর্ট' },
    { id: 'categoriesPage', icon: '🏷️', label: 'ক্যাটাগরি' },
    { id: 'recurringPage', icon: '🔄', label: 'পুনরাবৃত্তি' },
    { id: 'calculatorPage', icon: '🧮', label: 'ক্যালকুলেটর' },
    { id: 'savingsPage', icon: '🎯', label: 'সঞ্চয়' },
    { id: 'investmentPage', icon: '📈', label: 'বিনিয়োগ' },
    { id: 'loanPage', icon: '🤝', label: 'ধার-দেনা' }
];
function renderNav() {
    ...
    setTimeout(() => {
        ...
        if (ctxM) {
            ...
            monthlyChart = new Chart(ctxM, {
                type: 'bar', data: { labels: months, datasets: [{ label: 'আয়', data: incData, backgroundColor: 'rgba(16,185,129,0.7)', borderRadius: 8 }, { label: 'ব্যয়', data: expData, backgroundColor: 'rgba(239,68,68,0.7)', borderRadius: 8 }] },
...
function addEntry() {
    ...
    saveData(); showToast(currentType === 'income' ? 'আয় যোগ হয়েছে' : 'ব্যয় যোগ হয়েছে');
...
function renderReportsPage() {
    ...
    radioMonthly.addEventListener('change', () => { monthlyDiv.style.display = 'flex'; yearlyDiv.style.display = 'none'; generateReport(); });
    radioYearly.addEventListener('change', () => { monthlyDiv.style.display = 'none'; yearlyDiv.style.display = 'flex'; generateReport(); });
    ...
    if (isMonthly) {
        ...
        let html = `
            <div class="report-print-area" id="reportPrintArea">
                <div class="report-header"><div class="report-title">📊 মাসিক রিপোর্ট</div><div class="report-subtitle">${MONTHS_BN[month]} ${year} | ${new Date().toLocaleDateString('bn-BD')}</div></div>
                <div class="summary-cards"><div class="summary-item"><div class="summary-label">মোট আয়</div><div class="summary-value income">${fmt(totalInc)}</div></div><div class="summary-item"><div class="summary-label">মোট ব্যয়</div><div class="summary-value expense">${fmt(totalExp)}</div></div><div class="summary-item"><div class="summary-label">নেট সঞ্চয়</div><div class="summary-value net">${fmt(net)}</div></div></div>
                <div style="overflow-x:auto;"><table class="transaction-table"><thead><tr><th>তারিখ</th><th>বিবরণ</th><th>ক্যাটাগরি</th><th>পরিমাণ</th></tr></thead><tbody>${monthData.map(t => `<tr><td>${t.day} ${MONTHS_SHORT[t.month]}</td><td>${escapeHtml(t.description)}</td><td>${t.category}</td><td style="color:${t.type === 'income' ? '#10B981' : '#EF4444'}">${t.type === 'income' ? '+' : '-'}${fmt(t.amount)}</td></tr>`).join('') || '<tr><td colspan="4" class="empty-state">কোনো লেনদেন নেই</td></tr>'}</tbody>
                </table></div>
        ...
        let html = `
            <div class="report-print-area" id="reportPrintArea">
                <div class="report-header"><div class="report-title">📊 বার্ষিক রিপোর্ট ${year}</div><div class="report-subtitle">জানুয়ারি - ডিসেম্বর ${year}</div></div>
                <div class="summary-cards"><div class="summary-item"><div class="summary-label">মোট আয়</div><div class="summary-value income">${fmt(totalInc)}</div></div><div class="summary-item"><div class="summary-label">মোট ব্যয়</div><div class="summary-value expense">${fmt(totalExp)}</div></div><div class="summary-item"><div class="summary-label">নেট সঞ্চয়</div><div class="summary-value net">${fmt(net)}</div></div></div>
...
function addCat(type) {
    ...
    saveData(); renderCategoriesPage(); renderAddForm(); showToast(`"${name}" যোগ হয়েছে`);
    inp.value = '';
}
function removeCat(type, name) {
    if (confirm(`"${name}" ক্যাটাগরি মুছবেন?`)) {
        ...
        saveData(); renderCategoriesPage(); renderAddForm(); renderDashboard(); renderTransactions(); showToast(`"${name}" মুছে ফেলা হয়েছে`);
    }
}
function addPayment() {
    ...
    savePaymentMethods(); renderCategoriesPage(); renderAddForm(); showToast(`"${name}" যোগ হয়েছে`);
    inp.value = '';
}
function removePayment(name) {
    if (confirm(`"${name}" পেমেন্ট পদ্ধতি মুছবেন?`)) {
        ...
        data.forEach(d => { if (d.payment === name) d.payment = 'নগদ'; });
        savePaymentMethods(); saveData(); renderCategoriesPage(); renderAddForm(); renderDashboard(); renderTransactions(); showToast(`"${name}" মুছে ফেলা হয়েছে`);
    }
}

// ========== পুনরাবৃত্তি (Recurring) ==========
    ...
    const html = `
        <div class="card">
            ...
            <div style="display:flex; gap:15px; margin-top:20px;">
                <button class="btn-gradient-primary" id="addRecurringBtn" style="flex:1;"><i class="fas fa-plus-circle"></i> + নতুন নিয়ম</button>
                <button class="btn-gradient-primary" id="checkNowBtn" style="flex:1;"><i class="fas fa-sync"></i> এখনই চেক করুন</button>
            </div>
        </div>
    `;
    ...
    container.innerHTML = recurringRules.map((rule, idx) => `
        <div class="recurring-item">
            ...
            <div class="recurring-meta">
                <span>${rule.frequency === 'weekly' ? '📅 সাপ্তাহিক' : rule.frequency === 'monthly' ? '📆 মাসিক' : '📅 বার্ষিক'}</span>
                <span><i class="far fa-calendar-alt"></i> পরবর্তী: ${rule.nextDate || 'নির্ধারিত নয়'}</span>
                <span class="category-badge">${escapeHtml(rule.category)}</span>
            </div>
            <div class="recurring-actions">
                <button class="btn-edit-recurring" onclick="showRecurringModal(${idx})"><i class="fas fa-edit"></i> সম্পাদনা</button>
                <button class="btn-delete-recurring" onclick="deleteRecurring(${idx})"><i class="fas fa-trash"></i> মুছুন</button>
            </div>
        </div>
    `).join('');
}
    ...
    Swal.fire({
        ...
        html: `
            ...
            <div class="outline-input">
                ...
                <label for="recCategory">🏷️ ক্যাটাগরি</label>
            </div>
            <div class="outline-input">
                ...
                <label for="recPayment">💳 পেমেন্ট মাধ্যম</label>
            </div>
            <div class="outline-input">
                ...
                <label for="recFrequency">🔄 পুনরাবৃত্তি সময়কাল</label>
            </div>
            <div class="outline-input">
                ...
                <label for="recNextDate">📅 পরবর্তী কার্যকর তারিখ</label>
            </div>
            <div class="outline-input">
                <textarea id="recNote" rows="2" placeholder=" ">${rule ? rule.note || '' : ''}</textarea>
                <label for="recNote">📝 নোট (ঐচ্ছিক)</label>
            </div>
        `,
        showCancelButton: true,
        ...
        preConfirm: () => {
            ...
            if (!type) return Swal.showValidationMessage('লেনদেনের ধরন নির্বাচন করুন');
            ...
            if (!category) return Swal.showValidationMessage('ক্যাটাগরি নির্বাচন করুন');
            if (!payment) return Swal.showValidationMessage('পেমেন্ট মাধ্যম নির্বাচন করুন');
            if (!frequency) return Swal.showValidationMessage('পুনরাবৃত্তি সময়কাল নির্বাচন করুন');
            if (!nextDate) return Swal.showValidationMessage('পরবর্তী কার্যকর তারিখ দিন');
...
function renderSavingsPage() {
    ...
    const html = `
        ...
        <button class="btn-gradient-primary" onclick="addSavingsGoal()"><i class="fas fa-plus-circle"></i> নতুন সঞ্চয় লক্ষ্য</button>
    `;
...
function renderSavingsList() {
    ...
    if (filtered.length === 0) { container.innerHTML = '<div class="empty-state">কোনো সঞ্চয় লক্ষ্য নেই</div>'; return; }
...
function addSavingsGoal() {
    Swal.fire({
        title: '🎯 সঞ্চয় লক্ষ্য যোগ করুন',
        html: `
            <div class="outline-input"><input type="text" id="goalName" placeholder=" "><label for="goalName">🎯 সঞ্চয়ের নাম</label></div>
            <div class="outline-input"><input type="number" id="goalTarget" placeholder=" "><label for="goalTarget">💰 লক্ষ্যের পরিমাণ (টাকা)</label></div>
            <div class="outline-input"><input type="number" id="initialDeposit" placeholder=" "><label for="initialDeposit">💵 প্রাথমিক জমা (ঐচ্ছিক)</label></div>
            <div class="outline-input"><input type="date" id="startDate" placeholder=" "><label for="startDate">📅 শুরুর তারিখ</label></div>
        `,
        showCancelButton: true, confirmButtonText: 'যোগ করুন', cancelButtonText: 'বাতিল',
        customClass: { popup: 'responsive-payment-popup' },
        preConfirm: () => { let name = document.getElementById('goalName').value.trim(), target = parseFloat(document.getElementById('goalTarget').value); if (!name || !target) Swal.showValidationMessage('নাম ও পরিমাণ দিন'); return { name, target, initialDeposit: parseFloat(document.getElementById('initialDeposit').value) || 0, startDate: document.getElementById('startDate').value }; }
    }).then(res => { if (res.isConfirmed) { savingsGoals.push(res.value); saveSavingsGoals(); renderSavingsPage(); showToast('✅ সঞ্চয় লক্ষ্য যোগ হয়েছে'); } });
}
function addToSavings(idx) {
    Swal.fire({ title: '💰 টাকা যোগ করুন', input: 'number', inputPlaceholder: 'পরিমাণ (৳)', showCancelButton: true, confirmButtonText: 'যোগ করুন', preConfirm: (amt) => { if (!amt || amt <= 0) Swal.showValidationMessage('সঠিক পরিমাণ দিন'); return parseFloat(amt); } })
        .then(res => { if (res.isConfirmed) { data.unshift({ id: Date.now(), type: 'expense', amount: res.value, description: `সঞ্চয়: ${savingsGoals[idx].name}`, category: 'সঞ্চয়', payment: 'নগদ', note: '', day: now.getDate(), month: currentMonth, year: currentYear }); saveData(); renderDashboard(); renderTransactions(); renderSavingsPage(); showToast(`${fmt(res.value)} সঞ্চয়ে যোগ হয়েছে`); } });
}
function withdrawFromSavings(idx) {
    Swal.fire({ title: '💰 টাকা তোলুন', input: 'number', inputPlaceholder: 'পরিমাণ (৳)', showCancelButton: true, confirmButtonText: 'তুলুন', preConfirm: (amt) => { if (!amt || amt <= 0) Swal.showValidationMessage('সঠিক পরিমাণ দিন'); return parseFloat(amt); } })
        .then(res => { if (res.isConfirmed) { data.unshift({ id: Date.now(), type: 'income', amount: res.value, description: `সঞ্চয় থেকে উত্তোলন: ${savingsGoals[idx].name}`, category: 'অন্যান্য আয়', payment: 'নগদ', note: '', day: now.getDate(), month: currentMonth, year: currentYear }); saveData(); renderDashboard(); renderTransactions(); renderSavingsPage(); showToast(`${fmt(res.value)} উত্তোলন করা হয়েছে`); } });
}
function deleteSavingsGoal(idx) { if (confirm('মুছবেন?')) { savingsGoals.splice(idx, 1); saveSavingsGoals(); renderSavingsPage(); showToast('🗑️ মুছে ফেলা হয়েছে'); } }

// ========== বিনিয়োগ ==========
function renderInvestmentPage() {
    const totalInv = investments.reduce((s, i) => s + i.amount, 0);
    const html = `<div class="investment-stats-card"><div class="savings-amount">${fmt(totalInv)}</div><div class="savings-label">মোট বিনিয়োগ</div></div><div id="investmentsList"></div><button class="btn-gradient-primary" onclick="addInvestment()"><i class="fas fa-plus-circle"></i> নতুন বিনিয়োগ যুক্ত করুন</button>`;
...
function renderInvestmentsList() {
    ...
    if (!container) return;
    if (investments.length === 0) { container.innerHTML = '<div class="empty-state">কোনো বিনিয়োগ নেই</div>'; return; }
    container.innerHTML = investments.map((inv, idx) => `<div class="investment-card"><div class="investment-header"><span class="investment-name">${escapeHtml(inv.name)}</span><span class="investment-type">${inv.type || 'সাধারণ'}</span></div><div class="investment-details"><span>পরিমাণ: ${fmt(inv.amount)}</span><span>হার: ${inv.interest || 0}%</span></div><div class="investment-maturity"><i class="fas fa-calendar-alt"></i> শুরুর তারিখ: ${inv.startDate || 'শুরুর তারিখ'} | মেয়াদ উত্তীর্ণের তারিখ: ${inv.maturityDate || 'শেষের তারিখ'}</div><div class="investment-actions"><button class="btn-edit-invest" onclick="editInvestment(${idx})"><i class="fas fa-edit"></i> সম্পাদনা</button><button class="btn-delete-invest" onclick="deleteInvestment(${idx})"><i class="fas fa-trash"></i> মুছুন</button></div></div>`).join('');
}
function addInvestment() {
    Swal.fire({
        title: '📈 নতুন বিনিয়োগ',
        html: `
            <div class="outline-input"><input type="text" id="invName" placeholder=" "><label for="invName">📊 বিনিয়োগের নাম</label></div>
            <div class="outline-input"><select id="invType"><option value=""></option><option value="ডিপিএস">ডিপিএস</option><option value="এফডিআর">এফডিআর</option><option value="শেয়ার">শেয়ার</option><option value="সঞ্চয়পত্র">সঞ্চয়পত্র</option><option value="অন্যান্য">অন্যান্য</option></select><label for="invType">🏷️ বিনিয়োগের ধরন</label></div>
            <div class="outline-input"><input type="number" id="invAmount" placeholder=" "><label for="invAmount">💰 পরিমাণ (টাকা)</label></div>
            <div class="outline-input"><input type="number" id="invInterest" placeholder=" "><label for="invInterest">📈 সুদহার (%)</label></div>
            <div class="outline-input"><input type="date" id="invStartDate" placeholder=" "><label for="invStartDate">📅 শুরুর তারিখ</label></div>
            <div class="outline-input"><input type="date" id="invMaturityDate" placeholder=" "><label for="invMaturityDate">📅 মেয়াদ উত্তীর্ণের তারিখ</label></div>
        `,
        showCancelButton: true, confirmButtonText: 'যোগ করুন', cancelButtonText: 'বাতিল',
        customClass: { popup: 'responsive-payment-popup' },
        preConfirm: () => { let name = document.getElementById('invName').value.trim(), amt = parseFloat(document.getElementById('invAmount').value); if (!name || !amt) Swal.showValidationMessage('নাম ও পরিমাণ দিন'); return { name, type: document.getElementById('invType').value, amount: amt, interest: parseFloat(document.getElementById('invInterest').value) || 0, startDate: document.getElementById('invStartDate').value, maturityDate: document.getElementById('invMaturityDate').value }; }
    }).then(res => { if (res.isConfirmed) { investments.push(res.value); saveInvestments(); renderInvestmentPage(); showToast('✅ বিনিয়োগ যোগ হয়েছে'); } });
}
function editInvestment(idx) {
    const inv = investments[idx];
    Swal.fire({
        title: '✏️ বিনিয়োগ সম্পাদনা',
        html: `
            <div class="outline-input"><input type="text" id="invName" value="${escapeHtml(inv.name)}" placeholder=" "><label for="invName">📊 বিনিয়োগের নাম</label></div>
            <div class="outline-input"><select id="invType"><option value=""></option><option value="ডিপিএস" ${inv.type === 'ডিপিএস' ? 'selected' : ''}>ডিপিএস</option><option value="এফডিআর" ${inv.type === 'এফডিআর' ? 'selected' : ''}>এফডিআর</option><option value="শেয়ার" ${inv.type === 'শেয়ার' ? 'selected' : ''}>শেয়ার</option><option value="সঞ্চয়পত্র" ${inv.type === 'সঞ্চয়পত্র' ? 'selected' : ''}>সঞ্চয়পত্র</option><option value="অন্যান্য" ${inv.type === 'অন্যান্য' ? 'selected' : ''}>অন্যান্য</option></select><label for="invType">🏷️ বিনিয়োগের ধরন</label></div>
            <div class="outline-input"><input type="number" id="invAmount" value="${inv.amount}" placeholder=" "><label for="invAmount">💰 পরিমাণ (টাকা)</label></div>
            <div class="outline-input"><input type="number" id="invInterest" value="${inv.interest}" placeholder=" "><label for="invInterest">📈 সুদহার (%)</label></div>
            <div class="outline-input"><input type="date" id="invStartDate" value="${inv.startDate || ''}" placeholder=" "><label for="invStartDate">📅 শুরুর তারিখ</label></div>
            <div class="outline-input"><input type="date" id="invMaturityDate" value="${inv.maturityDate || ''}" placeholder=" "><label for="invMaturityDate">📅 মেয়াদ উত্তীর্ণের তারিখ</label></div>
        `,
        showCancelButton: true, confirmButtonText: 'আপডেট', cancelButtonText: 'বাতিল',
        customClass: { popup: 'responsive-payment-popup' },
        preConfirm: () => {
            investments[idx] = {
            ...
            saveInvestments(); renderInvestmentPage(); showToast('✅ আপডেট হয়েছে');
        }
    });
}
function deleteInvestment(idx) { if (confirm('মুছবেন?')) { investments.splice(idx, 1); saveInvestments(); renderInvestmentPage(); showToast('🗑️ মুছে ফেলা হয়েছে'); } }
...
function renderLoanPage() {
    ...
    const html = `
        ...
        <button class="btn-gradient-primary" onclick="openModernLoanModal()"><i class="fas fa-plus-circle"></i> নতুন ধার-দেনা যোগ করুন</button>
    `;
        ...
        return `
            <div class="loan-modern-card" onclick="openLoanDetailModal(${idx})">
                ...
                <div class="loan-actions">
                    <button class="btn-edit-loan" onclick="openEditLoanModal(${idx})"><i class="fas fa-edit"></i> সম্পাদনা</button>
                    <button class="btn-delete-loan" onclick="deleteLoanItem(${idx})"><i class="fas fa-trash"></i> মুছুন</button>
                </div>
            </div>
        `;
    }).join('');
...
function deleteLoanItem(idx) {
    if (confirm('❓ এই ধার-দেনাটি মুছে ফেলতে চান?')) {
        loans.splice(idx, 1);
        saveLoans();
        renderLoanPage();
        showToast('🗑️ ধার-দেনা মুছে ফেলা হয়েছে');
    }
}
    ...
    Swal.fire({
        ...
        html: `
            <div style="background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 24px; padding: 20px; margin-bottom: 20px; text-align: center; color: white;">
                <div style="font-size: 14px; opacity: 0.9;">বাকি পরিশোধযোগ্য</div>
            ...
            <div class="outline-input"><input type="number" id="payAmount" placeholder=" "><label for="payAmount">💰 পরিশোধের পরিমাণ (৳)</label></div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
                <div class="outline-input"><input type="date" id="payDate" value="${new Date().toISOString().split('T')[0]}" placeholder=" "><label for="payDate">📅 পরিশোধের তারিখ</label></div>
                <div class="outline-input"><select id="payMethod"><option value=""></option>${paymentMethods.map(p => `<option>${p}</option>`).join('')}</select><label for="payMethod">💳 অ্যাকাউন্ট</label></div>
            </div>
            <div style="margin-top: 8px;">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; margin-bottom: 8px;"><input type="checkbox" id="includeLateFee"> ⚠️ লেট ফি / জরিমানা যোগ করুন</label>
                <div class="outline-input" id="lateFeeContainer" style="display: none;"><input type="number" id="lateFeeVal" placeholder=" "><label for="lateFeeVal">💰 জরিমানার পরিমাণ (৳)</label></div>
            </div>
        `,
        showCancelButton: true,
        ...
        preConfirm: () => {
            ...
            if (!method) Swal.showValidationMessage('অ্যাকাউন্ট নির্বাচন করুন');
        ...
        if (res.isConfirmed) {
            ...
            if (lateFee > 0) {
                loan.amount += lateFee;
                loan.transactions.push({ id: Date.now() + 1, amount: lateFee, date, type: 'late_fee', note: 'জরিমানা' });
            }
...
function openEditLoanModal(idx) {
    ...
    Swal.fire({
        ...
        html: `
            <div class="outline-input"><input type="text" id="editName" value="${escapeHtml(loan.name)}" placeholder=" "><label for="editName">👤 নাম</label></div>
            <div class="outline-input"><input type="number" id="editAmount" value="${loan.amount}" placeholder=" "><label for="editAmount">💰 মোট পরিমাণ (টাকা)</label></div>
            <div class="outline-input"><select id="editType"><option value=""></option><option value="give" ${loan.type === 'give' ? 'selected' : ''}>আমি দিয়েছি</option><option value="take" ${loan.type === 'take' ? 'selected' : ''}>আমি নিয়েছি</option></select><label for="editType">🔄 ধরণ</label></div>
            <div class="outline-input"><input type="date" id="editLoanDate" value="${loan.loanDate || ''}" placeholder=" "><label for="editLoanDate">📅 দেওয়ার তারিখ</label></div>
            <div class="outline-input"><input type="date" id="editDueDate" value="${loan.dueDate || ''}" placeholder=" "><label for="editDueDate">📅 শেষ তারিখ</label></div>
            <div class="outline-input"><select id="editStatus"><option value="pending" ${loan.status === 'pending' ? 'selected' : ''}>বাকি</option><option value="paid" ${loan.status === 'paid' ? 'selected' : ''}>পরিশোধিত</option></select><label for="editStatus">✅ স্ট্যাটাস</label></div>
        `,
...
function openModernLoanModal() {
    Swal.fire({
        title: '🤝 নতুন ধার-দেনা',
        html: `
            <div class="outline-input"><input type="text" id="loanName" placeholder=" "><label for="loanName">👤 ব্যক্তির নাম বা বিবরণ</label></div>
            <div class="outline-input"><input type="number" id="loanAmount" placeholder=" "><label for="loanAmount">💰 পরিমাণ (টাকা)</label></div>
            <div class="outline-input"><select id="loanType"><option value=""></option><option value="give">আমি ধার দিয়েছি</option><option value="take">আমি ধার নিয়েছি</option></select><label for="loanType">🔄 ধরণ</label></div>
            <div class="outline-input"><input type="date" id="loanDate" placeholder=" "><label for="loanDate">📅 দেওয়ার তারিখ</label></div>
            <div class="outline-input"><input type="date" id="dueDate" placeholder=" "><label for="dueDate">📅 শেষ তারিখ</label></div>
            <div class="outline-input"><textarea id="loanNote" rows="2" placeholder=" "></textarea><label for="loanNote">📝 নোট (ঐচ্ছিক)</label></div>
        `,
        showCancelButton: true,
        ...
        preConfirm: () => {
            ...
            if (!type) Swal.showValidationMessage('ধরণ নির্বাচন করুন');
            return { name, amount: amt, type, loanDate: document.getElementById('loanDate').value, dueDate: document.getElementById('dueDate').value, note: document.getElementById('loanNote').value, status: 'pending', paidAmount: 0, transactions: [] };
        }
    }).then(res => { if (res.isConfirmed) { loans.push(res.value); saveLoans(); renderLoanPage(); showToast('✅ ধার-দেনা যোগ হয়েছে'); } });
}
